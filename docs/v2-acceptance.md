...
| V2-06 | Duration prediction supplies hierarchical cold start, online update, P50/P80/confidence/evidence count, censored partial handling, pre-update error and rollback without silently replacing user estimates; scheduling uses selected P80 for high-uncertainty research and P50 for divisible low-pressure work while displaying alternatives |
...
| V2-20 | First release implements selected F3 60-second typed friction only behind all hard-action guards; selected R1 synchronously releases unconditionally, then best-effort commits/schedules the 10-minute break when Core is reachable; Emergency, real block, break denial and replan are never randomized |
...
| V2-23 | Detailed context reads the latest 72 hours by default; permanent summaries/profiles/revisions and user-selected indefinite raw evidence are encrypted/date-partitioned/exportable/deletable with capacity alerts; raw evidence stays out of ordinary prompts, camera frames never persist, and frozen-release test data is deleted |
...
| V2-28 | Initial research progress adapters cover experiment execution, code development, paper reading and research writing with typed progress/attempt semantics and no content capture |
| V2-29 | Before Controller source is copied, provenance records the user's private LifeOS-only copy/modify/build authorization, prohibits public redistribution, and retains every third-party notice |
